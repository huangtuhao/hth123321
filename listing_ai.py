...
- 如果用户要求修改某部分，只修改该部分，其他部分保持原样"""
    ...
    def get_base_context(self):
        """获取所有基础上下文信息"""
        reference_analysis = self.extract_reference_analysis()  # 新增：参考产品分析
        ...
        keyword_data = self.extract_section("🔑 关键词数据")

        context_parts = ["# 基础信息（所有阶段均可参考）\n"]

        # 参考产品分析（最重要，放在最前面）
        if reference_analysis:
            context_parts.append("## 参考产品分析（本产品的主要依���，务必参考）\n")
        ...
        if keyword_data:
            context_parts.append("## 关键词数据\n")
            context_parts.append(f"{keyword_data}\n")
...
基于图片规划，为每张图片设计专业的表达方式和AI绘图prompt。
