...
// @version      3.7
// @description  (V3.7) 重大功能更新！1. 新增【箱子合并】功能，自动合并内容相同的箱子，模板中可使用 `mergedBoxItems` 列表。2. 新增【货件全选】功能，方便处理多货件计划。
...
(function() {
    ...
    const CONFIG = {
        ...
        USER_TEMPLATE_KEY: 'LX_USER_TEMPLATE_V3_7', // 更新版本号
    ...
    const mainApp = {
        ...
        selectShipment(shipmentList) {
            return new Promise((resolve) => {
                ...
                const selectionModal = modal.create('lx-invoice-selection-modal', '选择要生成发票的货件', listHtml, footer);

                const allCheckbox = document.getElementById('lx-select-all-shipments');
                const itemCheckboxes = selectionModal.querySelectorAll('input[name="shipment-checkbox"]');
                ...
                itemCheckboxes.forEach(cb => {
                    cb.onchange = () => {
                        allCheckbox.checked = Array.from(itemCheckboxes).every(i => i.checked);
                    };
                });

                document.getElementById('lx-invoice-cancel-btn').onclick = () => { modal.hide('lx-invoice-selection-modal'); resolve(null); };
                document.getElementById('lx-invoice-confirm-btn').onclick = () => {
                    const selectedIndexes = Array.from(itemCheckboxes).filter(cb => cb.checked).map(cb => parseInt(cb.value));
                    if (selectedIndexes.length === 0) { notify.error('请至少选择一个货件！'); return; }
        ...
        async selectTemplate() { const lastSelected = await GM_getValue(CONFIG.LAST_SELECTED_KEY, CONFIG.BUILT_IN_TEMPLATES[0]?.name); const userTemplate = await GM_getValue(CONFIG.USER_TEMPLATE_KEY); const CUSTOM_TEMPLATE_NAME = "用户自定义模板"; return new Promise(resolve => { let optionsHtml = ''; CONFIG.BUILT_IN_TEMPLATES.forEach(t => { optionsHtml += `<option value="${t.name}" ${t.name === lastSelected ? 'selected' : ''}>${t.name}</option>`; }); if (userTemplate) { optionsHtml += `<option value="${CUSTOM_TEMPLATE_NAME}" ${CUSTOM_TEMPLATE_NAME === lastSelected ? 'selected' : ''}>${CUSTOM_TEMPLATE_NAME}</option>`; } const bodyHtml = `<label for="lx-template-select" class="lx-template-select-label">请选择一个发票模板:</label><select id="lx-template-select" class="lx-template-select">${optionsHtml}</select><p style="font-size: 12px; color: #909399; margin-top: -10px; margin-bottom: 20px;">选择后将自动保存为默认选项。</p><button id="lx-upload-new-btn" class="lx-invoice-modal-btn lx-template-upload-btn">上传新的自定义模板</button><input type="file" id="lx-template-file-input" style="display: none;" accept=".xlsx, .xls">`; const footer = `<button id="lx-template-cancel-btn" class="lx-invoice-modal-btn lx-invoice-modal-btn-secondary">取消</button><button id="lx-template-next-btn" class="lx-invoice-modal-btn lx-invoice-modal-btn-primary">下一步</button>`; const templateModal = modal.create('lx-template-select-modal', '选择发票模板', bodyHtml, footer); const fileInput = document.getElementById('lx-template-file-input'); document.getElementById('lx-upload-new-btn').onclick = () => fileInput.click(); fileInput.onchange = (e) => { const file = e.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = async (event) => { const base64 = event.target.result.substring(event.target.result.indexOf(',') + 1); await GM_setValue(CONFIG.USER_TEMPLATE_KEY, base64); notify.success('自定义模板上传成功！'); modal.hide('lx-template-select-modal'); resolve(this.selectTemplate()); }; reader.readAsDataURL(file); }; document.getElementById('lx-template-cancel-btn').onclick = () => { modal.hide('lx-template-select-modal'); resolve(null); }; document.getElementById('lx-template-next-btn').onclick = async () => { const selectedName = document.getElementById('lx-template-select').value; await GM_setValue(CONFIG.LAST_SELECTED_KEY, selectedName); if (selectedName === CUSTOM_TEMPLATE_NAME) { modal.hide('lx-template-select-modal'); resolve(userTemplate); return; } const selectedTemplateConfig = CONFIG.BUILT_IN_TEMPLATES.find(t => t.name === selectedName); if (!selectedTemplateConfig) { notify.error("选择的内置模板配置不存在！"); return; } const cache = await GM_getValue(CONFIG.BUILT_IN_CACHE_KEY, {}); if (cache[selectedTemplateConfig.url]) { notify.success(`已从缓存加载模板: ${selectedName}`); modal.hide('lx-template-select-modal'); resolve(cache[selectedTemplateConfig.url]); return; } modal.showLoading(`首次加载，正在从网络获取模板: ${selectedName}...`); try { const response = await this.fetchImage(selectedTemplateConfig.url); if (!response) throw new Error("下载失败，请检查URL或网络。"); const base64 = templateHelpers.arrayBufferToBase64(response); cache[selectedTemplateConfig.url] = base64; await GM_setValue(CONFIG.BUILT_IN_CACHE_KEY, cache); notify.success(`模板获取并缓存成功！`); modal.hide('lx-template-select-modal'); modal.hideLoading(); resolve(base64); } catch (error) { notify.error(`获取模板失败: ${error.message}`); modal.hideLoading(); } }; }); },
        
        // 【V3.7 更新】重构此函数，加入箱子合并逻辑
        async processTemplateAndDownload({ templateBase64, skuItems, globalInfo, vueInstance }) {
            ...
            for (const fbaId in groupedByFbaId) {
                ...
                for (const group of boxGroups.values()) {
                    ...
                    const consolidatedBoxId = `${representativeBox.id}-${representativeBox.id.slice(-1) * 1 + boxCount - 1}`;
                    ...
                    itemsInRepBox.forEach(item => {
                        mergedBoxItems.push({
                            ...item,
                            boxId: consolidatedBoxId, // 使用合并后的ID
                            boxCount: boxCount,      // 附加上箱子数量
                        });
                    });
                }
                // ---【箱子合并逻辑结束】---
                ...
                const initialContext = { 
                    ...
                    mergedBoxes: mergedBoxes, // 新增：合并后的箱子组
                    mergedBoxItems: mergedBoxItems // 新增：合并后的SKU行项目
                };
        ...
        async run() {
            try {
                ...
                if (finalData) { await this.processTemplateAndDownload({ templateBase64, ...finalData }); }
            } catch (error) { console.error("生成发票主流程失败:", error); notify.error(`操作失败: ${error.message}`); modal.hideLoading(); }
        }
    };
