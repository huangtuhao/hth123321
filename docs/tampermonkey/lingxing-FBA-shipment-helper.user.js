  ...
  const modal = {
    ...
    showLoading() {
      this.create(
        "lx-loading-modal",
        "正在处理中...",
        '<div style="text-align:center; padding: 40px 0;">请稍候，正在从服务器获取所有装箱数据...</div>',
      );
    },
    hide(id) {
  ...
  const mainApp = {
    async fetchInitialData() {
      try {
        ...
        if (!vueInstance.info) {
          await vueInstance.getDetail(
            new URLSearchParams(window.location.search).get("localTaskId"),
          );
        }

        const globalInfo = vueInstance.info;
        const shipmentList = document.querySelector("#pane-3 .info-container")
          ?.__vue__?.shipmentList;

        if (!globalInfo || !shipmentList) {
          throw new Error(
            "无法获取 globalInfo 或 shipmentList，可能未切换到第三步。",
          );
        }
        ...
        notify.error(
          '请确认已经切换到第3步："配送服务"，如果未切换，请点击头部"配送服务"文字进行切换',
        );
        return null;
      }
    },
    ...
    async fetchPackingDetails(vueInstance, shipment) {
      try {
        const { code, msg, data } = await vueInstance.$gwPost(
        ...
        if (code === 1 && data) {
          ...
          throw new Error(
            `货件 ${shipment.shipmentId} 的装箱数据请求失败: ${msg || "未知错误"}`,
          );
        }
      } catch (error) {
    ...
    async handleShortNames(initialData) {
      ...
      document.getElementById("lx-cancel-btn").onclick = () =>
      ...
      document.getElementById("lx-save-btn").onclick = async () => {
        ...
        modal.showLoading();
        try {
          const packingPromises = initialData.shipmentList.map((s) =>
            this.fetchPackingDetails(initialData.vueInstance, s),
          );
    ...
    generateFinalOutput(initialData, allPackingDetails, shortNameMap) {
      ...
      shipmentList.forEach((shipment, index) => {
        ...
        const contentSummary = Array.from(aggregatedBoxCounts.entries())
          ...
          .join(" ");
        const shopName = globalInfo.sellerName
          .replace(/-[A-Z]{2,}(\s*-\s*\S+)?$/, "")
          .trim();
        const creatorName =
          globalInfo.createByName ||
          document.querySelector("#app")?.__vue__?.$store?.state?.userName;
        const date = new Date()
        ...
        packingVos.forEach((box, boxIndex) => {
          ...
          if (!currentGroup) {
            ...
            const shortName =
              shortNameMap[currentGroup.sku] || currentGroup.sku;
