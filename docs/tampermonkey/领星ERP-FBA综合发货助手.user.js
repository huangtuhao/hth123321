...
(function() {
    ...
    function addGlobalStyle() { 
        ...
        `; 
        const styleNode = document.createElement('style'); styleNode.appendChild(document.createTextNode(css)); document.head.appendChild(styleNode); 
    }
    ...
    const UI = {
        createModal(id, title, bodyContent, footerButtons, isLarge = false) { 
            ...
            overlay.querySelector('.lx-modal-content').addEventListener('click', e => e.stopPropagation()); 
            return overlay; 
        },
        hideModal(id) { const el = document.getElementById(id); if (el) el.remove(); },
        showLoading(msg = '处理中...') { this.createModal('lx-loading', '请稍候', `<div style="text-align:center; padding: 40px 0; font-size: 16px; color: #409EFF;">加载中：${msg}</div>`); },
        hideLoading() { this.hideModal('lx-loading'); },
        notify: {
            _show(type, msg) {
                const vue = document.querySelector('#app')?.__vue__;
                if (vue && vue.$message) { vue.$message[type](msg); } else { alert(msg); }
            },
    ...
    const SharedUtils = {
        ...
        generateFilename(shipmentName, fbaId) {
            ...
            let suffix = shipmentName.substring(anchorMatch.index + anchorMatch[0].length);
    ...
    const DataService = {
        async getPageContext() {
            try {
                ...
                if (localTaskId && !vueInstance.info) { await vueInstance.getDetail(localTaskId); }
                const globalInfo = vueInstance.info;
                const shipmentList = document.querySelector('#pane-3 .info-container')?.__vue__?.shipmentList || document.querySelector('#pane-3 .sta-delivery-service-wrapper')?.__vue__?.shipmentList;
        ...
        async getPackingDetails(shipment, vueInstance) {
            try {
                const res = await vueInstance.$gwPost("/amz-sta-server/inbound-packing/getShipmentPackingDetail", {
                    shipmentId: shipment.shipmentId, inboundPlanId: shipment.inboundPlanId, sid: shipment.sid || vueInstance.info.sid
                });
                if (res.code === 1 && res.data) return res.data;
                throw new Error(res.msg || 'API返回无数据');
            } catch (err) {
                console.error("装箱数据获取失败:", err);
                UI.notify.error(`获取货件 ${shipment.shipmentConfirmationId} 装箱详情失败`);
                return null;
            }
        },
        async getProductDetails(globalInfo, vueInstance) {
            ...
            const promises = items.map(async (item, i) => {
                ...
                try {
                    const res = await vueInstance.$gateAxios.get(`/api/product/info?id=${item.productId}&req_time_sequence=${seq}`);
                    if (res.data.code === 1 && res.data.info) return { id: item.productId, details: res.data.info };
                } catch (e) {}
                return { id: item.productId, details: null };
            });
            const results = await Promise.all(promises);
            const map = new Map();
            results.forEach(r => { if (r.details) map.set(r.id, r.details); });
            return map;
        }
    };
    ...
    const InvoiceModule = {
        templateHelpers: {
            ...
            runPreprocessor(scriptContent, rawContext) { 
                return new Promise((resolve) => { 
                    if (!scriptContent || typeof scriptContent !== 'string' || !scriptContent.trim().startsWith('// js-script')) { resolve(rawContext); return; } 
                    try { 
                        const processor = new Function('context', `${scriptContent}\nreturn context;`); 
                        ...
                        console.error("A1预处理报错:", e); resolve(rawContext); 
                    } 
                }); 
            },
            getProperty(obj, path) { if (typeof path !== 'string') return obj ? obj[path] : undefined; return path.split('.').reduce((o, k) => (o || {})[k], obj); },
            evaluateInScope(expr, scope) { try { return new Function(...Object.keys(scope), `'use strict'; return (${expr});`)(...Object.values(scope)); } catch (e) { return ''; } },
            evaluateExpression(expr, scope) { if (typeof expr !== 'string' || !expr.includes('{{')) return expr; const singleMatch = expr.trim().match(/^{{(.*)}}$/); if (singleMatch) return this.evaluateInScope(singleMatch[1].trim(), scope); return expr.replace(/{{(.*?)}}/g, (m, exp) => { const v = this.evaluateInScope(exp.trim(), scope); return v == null ? '' : v; }); }
        },
        ...
        selectShipment(shipmentList) {
            return new Promise((resolve) => {
                ...
                shipmentList.forEach((s, i) => { listHtml += `<li><label><input type="checkbox" name="lx-shipment-cb" value="${i}" checked><div><strong>${s.shipmentConfirmationId}</strong> (${s.warehouseId})<br><small style="color:#909399;">SKU: ${s.mskuCount} 款 | 总箱数: ${s.boxQuantity}</small></div></label></li>`; }); 
                listHtml += '</ul>';
                ...
                const modal = UI.createModal('lx-inv-sel-modal', '第一步：选择要导出的货件', listHtml, footer);
                
                const allCb = document.getElementById('lx-select-all');
                const itemCbs = modal.querySelectorAll('input[name="lx-shipment-cb"]');
                allCb.onchange = e => itemCbs.forEach(cb => cb.checked = e.target.checked);
                itemCbs.forEach(cb => cb.onchange = () => allCb.checked = Array.from(itemCbs).every(i => i.checked));
                
                document.getElementById('lx-inv-cancel').onclick = () => { UI.hideModal('lx-inv-sel-modal'); resolve(null); };
                document.getElementById('lx-inv-next').onclick = () => {
                    const selected = Array.from(itemCbs).filter(cb => cb.checked).map(cb => shipmentList[parseInt(cb.value)]);
                    if (selected.length === 0) return UI.notify.warning('请至少选择一个货件');
        ...
        async selectTemplate() {
            return new Promise(async (resolve) => {
                const lastSel = await GM_getValue(CONFIG.LAST_SELECTED_KEY, CONFIG.BUILT_IN_TEMPLATES[0]?.name);
                const userTpl = await GM_getValue(CONFIG.USER_TEMPLATE_KEY);
                let opts = '';
                CONFIG.BUILT_IN_TEMPLATES.forEach(t => { opts += `<option value="${t.name}" ${t.name === lastSel ? 'selected' : ''}>${t.name}</option>`; });
                if (userTpl?.data) { opts += `<option value="${CONFIG.CUSTOM_TEMPLATE_IDENTIFIER}" ${lastSel === CONFIG.CUSTOM_TEMPLATE_IDENTIFIER ? 'selected' : ''}>${userTpl.name} (自定义)</option>`; }
                ...
                const modal = UI.createModal('lx-tpl-modal', '第二步：选择发票模板', html, footer);
                ...
                const updateBtns = () => {
                    ...
                    if (select.value === CONFIG.CUSTOM_TEMPLATE_IDENTIFIER) {
                        const delBtn = document.createElement('button'); delBtn.innerText = '删除'; delBtn.className = 'lx-modal-btn lx-modal-btn-danger';
                        delBtn.onclick = async () => { if(confirm('删除自定义模板？')) { await GM_setValue(CONFIG.USER_TEMPLATE_KEY, null); UI.notify.success('已删除'); UI.hideModal('lx-tpl-modal'); resolve(this.selectTemplate()); }};
                        actions.appendChild(delBtn);
                    } else {
                        const updBtn = document.createElement('button'); updBtn.innerText = '强制更新'; updBtn.className = 'lx-modal-btn lx-modal-btn-warning';
                        updBtn.onclick = async () => { const cache = await GM_getValue(CONFIG.BUILT_IN_CACHE_KEY, {}); const cfg = CONFIG.BUILT_IN_TEMPLATES.find(t=>t.name===select.value); if(cfg && cache[cfg.url]) { delete cache[cfg.url]; await GM_setValue(CONFIG.BUILT_IN_CACHE_KEY, cache); UI.notify.success('缓存已清空，下次将重新下载'); }};
                        actions.appendChild(updBtn);
                    }
                };
                select.onchange = updateBtns; updateBtns();

                document.getElementById('lx-upload-btn').onclick = () => document.getElementById('lx-file-input').click();
                document.getElementById('lx-file-input').onchange = (e) => {
                    const file = e.target.files[0]; if(!file) return;
                    const reader = new FileReader();
                    reader.onload = async (ev) => {
                        const base64 = ev.target.result.split(',')[1];
                        await GM_setValue(CONFIG.USER_TEMPLATE_KEY, { name: file.name, data: base64 });
                        await GM_setValue(CONFIG.LAST_SELECTED_KEY, CONFIG.CUSTOM_TEMPLATE_IDENTIFIER);
                        UI.hideModal('lx-tpl-modal'); UI.notify.success('上传成功'); resolve(this.selectTemplate());
                    }; reader.readAsDataURL(file);
                };
                
                document.getElementById('lx-tpl-cancel').onclick = () => { UI.hideModal('lx-tpl-modal'); resolve(null); };
                document.getElementById('lx-tpl-next').onclick = async () => {
                    const val = select.value; await GM_setValue(CONFIG.LAST_SELECTED_KEY, val);
                    if (val === CONFIG.CUSTOM_TEMPLATE_IDENTIFIER) {
                        const t = await GM_getValue(CONFIG.USER_TEMPLATE_KEY); UI.hideModal('lx-tpl-modal'); resolve(t?.data); return;
                    }
                    const cfg = CONFIG.BUILT_IN_TEMPLATES.find(t => t.name === val);
                    const cache = await GM_getValue(CONFIG.BUILT_IN_CACHE_KEY, {});
                    if (cache[cfg.url]) { UI.hideModal('lx-tpl-modal'); resolve(cache[cfg.url]); return; }
                    ...
                    if (buf) {
                        ...
                        cache[cfg.url] = b64; await GM_setValue(CONFIG.BUILT_IN_CACHE_KEY, cache);
                        UI.hideLoading(); UI.hideModal('lx-tpl-modal'); resolve(b64);
                    } else {
                        UI.hideLoading(); UI.notify.error('模板下载失败');
                    }
                };
            });
        },
        ...
        async showConfirmModal(shipments, globalInfo, productMap) {
            ...
            const allItems = []; let rows = '';
            
            shipments.forEach(s => {
                s.itemList.forEach(i => {
                    const pItem = globalInfo.inboundPlanItemVOS.find(p => p.sku === i.sku); if(!pItem) return;
                    const pDetail = productMap.get(pItem.productId) || {}; 
                    const sData = savedData[pItem.sku] || {};
                    const boxCount = Math.round(i.quantity / pItem.quantityInBox) || 0; if (boxCount === 0) return;
                    ...
                    const item = {
                        shipment: s, planItem: pItem, fbaId: s.shipmentConfirmationId, boxCount,
                        ...
                        hscode: sData.hscode || clearance.customs_clearance_hs_code || declaration.customs_declaration_hs_code || pDetail.bg_export_hs_code || '',
                        price: sData.price || clearance.customs_clearance_price || declaration.customs_import_price || pDetail.bg_customs_import_price || '0.00',
                        material: sData.material || clearance.customs_clearance_material || pDetail.cg_product_material || '',
                        usage: sData.usage || clearance.customs_clearance_usage || '',
                        brand: sData.brand || pDetail.brand_name || '', model: sData.model || pDetail.model || pItem.sku
                    };
                    allItems.push(item);
                });
            });
            ...
            const batchBtn = (field) => `<br><a href="javascript:void(0)" class="lx-batch-btn" data-batch="${field}" style="font-size:12px; font-weight:normal; color:#409EFF; text-decoration:none; display:inline-block; margin-top:4px;">[批量修改]</a>`;
            ...
            return new Promise(resolve => {
                const modal = UI.createModal('lx-cfm-modal', '第三步：补充并确认报关资料', html, footer, true);
                
                modal.querySelectorAll('.lx-batch-btn').forEach(btn => {
                    btn.onclick = (e) => {
                        const field = e.target.dataset.batch;
                        const fieldName = e.target.parentElement.childNodes[0].textContent.trim(); 
                        const newVal = prompt(`请输入要批量应用到所有行的【${fieldName}】的值：\n(留空将清空该列)`);
                        if (newVal !== null) {
                            modal.querySelectorAll(`input[data-f="${field}"]`).forEach(input => input.value = newVal.trim());
                            UI.notify.success(`已将【${fieldName}】批量修改完毕`);
                        }
                    };
                });

                document.getElementById('lx-cfm-cancel').onclick = () => { UI.hideModal('lx-cfm-modal'); resolve(null); };
                document.getElementById('lx-cfm-go').onclick = async () => {
                    ...
                    modal.querySelectorAll('tbody tr').forEach(tr => {
                        ...
                        const fields = ['hscode', 'price', 'material', 'usage', 'brand', 'model'];
                        fields.forEach(f => {
                    ...
                    UI.hideModal('lx-cfm-modal'); resolve(allItems);
                };
            });
        },
        ...
        async generateExcel(base64, items, globalInfo, vueInstance) {
            ...
            const helpers = {
                convertToKg: (value, sourceUnit) => {
                    ...
                    const factors = { g: 0.001, gram: 0.001, grams: 0.001, 克: 0.001, kg: 1, kilogram: 1, kilograms: 1, 千克: 1, 公斤: 1, lb: 0.453592, lbs: 0.453592, pound: 0.453592, pounds: 0.453592, 磅: 0.453592, oz: 0.0283495, ounce: 0.0283495, ounces: 0.0283495, 盎司: 0.0283495 };
                    return factors[unit] !== undefined ? num * factors[unit] : num;
                }
            };

            const buf = Uint8Array.from(atob(base64), c => c.charCodeAt(0)).buffer;
            const grouped = items.reduce((acc, i) => { (acc[i.fbaId] = acc[i.fbaId] || { s: i.shipment, items: [] }).items.push(i); return acc; }, {});
            
            for (const fbaId in grouped) {
                const { s: shipment, items: groupItems } = grouped[fbaId];
                UI.showLoading(`正在处理 ${fbaId} - 获取装箱...`);
                const packData = await DataService.getPackingDetails(shipment, vueInstance);
                ...
                const boxItems = packData.shipmentPackingVOS.map(item => ({...item, ...skuMap.get(item.sku)}));
                ...
                boxItems.forEach(item => { 
                    if (!boxesMap.has(item.boxId)) { 
                        ...
                        boxesMap.set(item.boxId, { 
                            id: item.boxId, name: item.boxName, weight: item.weight, length: item.length, width: item.width, height: item.height, 
                            dimensions: `${item.length}*${item.width}*${item.height}`, contents: [] 
                        }); 
                    } 
                    boxesMap.get(item.boxId).contents.push({ 
                        sku: item.sku, msku: item.msku, fnsku: item.fnsku, quantity: item.quantityInBox, picUrl: item.url, ...skuMap.get(item.sku) 
                    }); 
                });
                const boxes = Array.from(boxesMap.values());

                const mergedBoxes = []; const mergedBoxItems = [];
                const groups = new Map();
                boxes.forEach(b => {
                    const cHash = JSON.stringify([...b.contents].sort((x,y)=>x.sku.localeCompare(y.sku)).map(c=>({sku:c.sku,q:c.quantity})));
                ...
                function processGrp(sub) {
                    ...
                    const rep = sub[0]; const c = sub.length;
                    const cId = c <= 1 ? rep.id : `${rep.id}-${getNum(rep.id)+c-1}`;
                    mergedBoxes.push({ count: c, totalWeight: rep.weight * c, name: sub.map(b=>b.id).join(','), ...rep });
                    
                    // 核心修复5：从富含所有商品信息的 boxItems 中过滤，而不是原始的 packingData
                    const itemsInRepBox = boxItems.filter(i=>i.boxId===rep.id);
                    itemsInRepBox.forEach(i => mergedBoxItems.push({...i, boxId: cId, boxCount: c}));
                }
                ...
                const ctx = { 
                    shipment: { 
                        id: packData.shipmentId, totalBoxNum: boxes.length, 
                        totalWeight: boxes.reduce((sum, box) => sum + box.weight, 0).toFixed(2), 
                        address: shipment.address, warehouseId: shipment.warehouseId 
                    }, 
                    rawShipment: shipment, boxes, boxItems, skuItems: groupItems, 
                    globalInfo, mergedBoxes, mergedBoxItems, helpers 
                };
                ...
                if (typeof pre === 'string' && pre.startsWith('// js-script')) ws.getCell('A1').value = null;

                let block = null;
                ws.eachRow((row, rn) => { 
                    row.eachCell(cell => { 
                        const v = typeof cell.value === 'object' ? cell.value?.text : cell.value;
                        if (typeof v === 'string') {
                            const m = v.match(/^{{#each\s+([\w.]+)(?:\s+as\s+(\w+))?}}$/);
                            if (m) block = { start: rn, arr: m[1], alias: m[2]||'item', rows: [] };
                            else if (block && !block.end && v.trim() === '{{/each}}') block.end = rn;
                        }
                    }); 
                });
                ...
                ws.eachRow((row, rn) => {
                    ...
                    row.eachCell(cell => { if(typeof cell.value==='string') cell.value = this.templateHelpers.evaluateExpression(cell.value, finalCtx); });
                });

                if (block && block.end) {
                    for(let i=block.start+1; i<block.end; i++) {
                        const r = ws.getRow(i); const rm = { height: r.height, cells: [] };
                        r.eachCell({includeEmpty:true}, (c, cn) => rm.cells[cn] = { v: c.value, s: JSON.parse(JSON.stringify(c.style)) });
                        block.rows.push(rm);
                    }
                    const arr = this.templateHelpers.getProperty(finalCtx, block.arr) || [];
                    const pics = new Set(); const inserts = [];
                    ...
                    arr.forEach(itm => {
                        ...
                        block.rows.forEach(rm => {
                            ...
                            rm.cells.forEach((cm, cn) => {
                                ...
                                const imgM = typeof v==='string' ? v.match(/^{{@(.*)}}$/) : null;
                                if(imgM) { const u = this.templateHelpers.evaluateInScope(imgM[1], scope); if(u) pics.add(u); rowD[cn] = u; }
                                else rowD[cn] = this.templateHelpers.evaluateExpression(v, scope);
                            });
                            inserts.push(rowD);
                        });
                    });
                    ...
                    for (const url of pics) { const b = await SharedUtils.fetchImage(url); if(b) imgBufs.set(url, b); }
                    ...
                    arr.forEach(itm => {
                        block.rows.forEach(rm => {
                            const row = ws.getRow(block.start + offset); row.height = rm.height;
                            rm.cells.forEach((cm, cn) => {
                                const cell = row.getCell(cn); cell.style = cm.s;
                                const v = typeof cm.v==='object' ? cm.v?.text : cm.v;
                                if(typeof v==='string' && v.match(/^{{@(.*)}}$/)) {
                                    const u = cell.value; const ib = imgBufs.get(u);
                                    if (ib) {
                                        try { const iId = wb.addImage({buffer: ib, extension:'jpeg'}); ws.addImage(iId, { tl: {col:cn-1, row:row.number-1}, br: {col:cn, row:row.number}}); cell.value=null; } catch(e){}
                                    }
                                }
                            });
                            offset++;
        ...
        async run(contextData) {
            try {
                const shipments = await this.selectShipment(contextData.shipmentList); if(!shipments) return;
                const tpl = await this.selectTemplate(); if(!tpl) return;
                
                UI.showLoading('正在获取产品报关与图片信息...');
                const pMap = await DataService.getProductDetails(contextData.globalInfo, contextData.vueInstance);
                UI.hideLoading();
                
                const finalItems = await this.showConfirmModal(shipments, contextData.globalInfo, pMap);
                if (finalItems) {
                    await this.generateExcel(tpl, finalItems, contextData.globalInfo, contextData.vueInstance);
                }
            } catch (e) {
                console.error(e); UI.notify.error("生成失败: " + e.message);
            } finally {
                UI.hideLoading();
            }
        }
    ...
    const LabelModule = {
        async run(contextData) {
            ...
            const modal = UI.createModal('lx-lbl-modal', '定义箱唛简称', body, footer);
            document.getElementById('lx-lbl-cancel').onclick = () => UI.hideModal('lx-lbl-modal');
            document.getElementById('lx-lbl-go').onclick = async () => {
                const newMap = {};
                modal.querySelectorAll('input[data-sku]').forEach(inp => { if(inp.value.trim()) newMap[inp.dataset.sku] = inp.value.trim(); });
                await GM_setValue(CONFIG.SHORT_NAME_STORAGE_KEY, newMap);
                UI.hideModal('lx-lbl-modal');
                
                UI.showLoading('正在获取所有货件装箱明细...');
                try {
                    const promises = contextData.shipmentList.map(s => DataService.getPackingDetails(s, contextData.vueInstance));
                    const allPack = await Promise.all(promises);
                    this.renderOutput(contextData, allPack, newMap);
                } catch(e) { UI.notify.error(e.message); }
                finally { UI.hideLoading(); }
            };
        },

        renderOutput({globalInfo, shipmentList, vueInstance}, allPack, shortMap) {
            let names = []; let details = '';
            shipmentList.forEach((s, idx) => {
                const pVos = allPack[idx]?.shipmentPackingVOS; if(!pVos) return;
                
                const counts = new Map();
                pVos.forEach(b => { if(!counts.has(b.sku)) counts.set(b.sku,0); counts.set(b.sku, counts.get(b.sku)+1); });
                const summary = Array.from(counts.entries()).map(([sku, c]) => `${shortMap[sku]||sku}${c}箱`).join(' ');
                
                const shop = globalInfo.sellerName.replace(/-[A-Z]{2,}(\s*-\s*\S+)?$/, '').trim();
                const creator = globalInfo.createByName || vueInstance.$store?.state?.userName || '未知';
                const date = new Date().toLocaleDateString('zh-CN', {year:'2-digit',month:'2-digit',day:'2-digit'}).replace(/\//g,'');
                const dest = s.warehouseId; const fba = s.shipmentConfirmationId;
                names.push(`${shop}-${creator}-${date}-${summary}-${dest}-${fba}-${CONFIG.TEMPLATE_SUFFIX}`);
                ...
                pVos.forEach((b, i) => {
                    ...
                    else { details += `${cur.s}-${cur.e}：${shortMap[cur.sku]||cur.sku}\n`; cur = { sku: b.sku, s: n, e: n }; }
                    if(i === pVos.length-1) details += `${cur.s}-${cur.e}：${shortMap[cur.sku]||cur.sku}\n`;
                });
                details += `\n`;
            });
        ...
        openTab(names, details) {
            const namesHtml = names.map((n, i) => `<div style="display:flex;margin-bottom:10px;"><input type="text" id="n-${i}" value="${n}" style="flex:1;padding:10px;border:1px solid #dcdfe6;border-radius:4px 0 0 4px;"><button onclick="copy('n-${i}')" style="padding:0 20px;background:#409eff;color:#fff;border:none;border-radius:0 4px 4px 0;cursor:pointer;">复制</button></div>`).join('');
    ...
    const AppController = {
        init() {
            ...
            const checkPage = () => {
                ...
                btn.style.display = ['/erp/msupply/SendToAmazonDetail', '/erp/msupply/AddSendToAmazon', '/erp/msupply/editSendToAmazon'].some(v => url.includes(v)) ? 'block' : 'none';
            };

            const _push = history.pushState;
            history.pushState = function(...a) { const r = _push.apply(this, a); window.dispatchEvent(new Event('lx_url_change')); return r; };
            window.addEventListener('lx_url_change', checkPage);
            window.addEventListener('popstate', checkPage);
            
            const ob = new MutationObserver(() => { if(document.querySelector('.sta-detail')){ checkPage(); ob.disconnect(); }});
        ...
        async showMenu() {
            ...
            document.getElementById('lx-menu-cancel').onclick = () => UI.hideModal('lx-main-menu');
