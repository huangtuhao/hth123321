...
class ListingDesigner:
    ...
    def __init__(self):
        ...
        self.branch_a_stages = ["图片类型规划与卖点分配", "卖点表达方式设计", "主图+附图设计Brief生成"]
...
# 🎯 任务：图片类型规划与卖点分配（主图+6张附图，共7张）

你是经验丰富的亚马逊运营设计师。请基于品类竞品的成熟图片模板，为7张图片规划**图片类型、承载的卖点组合、以及说服逻辑**。

## 背景知识：亚马逊市场验证的常见图片类型

以下是经过大量A/B测试和销量验证沉淀的常见图片类型（不局限于此，可根据品类调整）：

| 图片类型 | 说明 | 信息密度 |
|----------|------|----------|
| **白底产品图** | 主图标配，展示产品全貌+核心功能预览（如使用演示、高度箭头等） | 中 |
| **尺寸/规格信息图** | 标注线+数据+多个规格细节，一图叠加多个技术卖点 | 高 |
| **细节特写信息图** | 多个小图组合（如四宫格细节），同时展示多个材质/工艺/功能细节 | 高 |
| **Ours vs Others 对比图** | 左右分栏或表格，直观展示多维度产品优势 | 高 |
| **痛点场景图** | 展示消费者痛点（如红色标记）+ 产品解决方案 | 中 |
| **多场景/多用途展示图** | 四宫格或多图拼接，展示产品在不同场景的使用 | 高 |
| **礼物/生活方式图** | 情感收尾，触发购买/送礼冲动 | 低 |

## ⚠️ 核心原则

1. **每张图有一个核心传达（core message）**：先想清楚这张图要让消费者产生什么感受/认知，再决定放哪些信息元素来支撑。不是往图里塞卖点，而是围绕核心信息组织细节
2. **信息元素数量由图片类型决定，不强制**：信息图/对比图天然承载多个细节（4-6个），场景图/礼物图天然信息密度低（1-2个），不要为了"多"而硬凑
3. **按消费者浏览心理旅程排序**（而非卖点重要性排序）：
   看到产品（吸引注意）→ 了解规格尺寸 → 确认品质细节 → 感受痛点解决 → 想象多种用途 → 触发购买/送礼冲动
4. **选择适合本品类的图片类型组合**：参考上表但不必拘泥，根据产品特点调整

## 输出要求（中文）

### 主图（第1张）
- **图片类型**：[选择适合的类型]
- **核心传达**：[这张图要让消费者产生什么感受/认知，一句话]
- **信息元素**：[画面中具体展示哪些细节/特征，数量由图片类型决定]
- **消费者心理目标**：[本图在浏览旅程中的角色]
...
### 排序逻辑说明
解释7张图的顺序如何对应消费者的浏览心理旅程（而非卖点重要性排序）

### 卖点覆盖检查表
| 卖点 | 出现在哪几张图（作为信息元素） | 是否有图以此为核心传达 |
确保所有核心卖点至少在1张图中被覆盖，重要卖点可在多张图中以不同角度出现。"""
...
⚠️ **每张图的图片类型、核心传达、信息元素必须严格遵循图片规划，不得更改。**

## 核心设计原则
- 围绕每张图的**核心传达**设计视觉表达，所有信息元素为核心传达服务
- 信息密度高的图片类型（规格图、细节图、对比图）需设计清晰的**信息布局**
- 参考竞品同类型图片的成熟做法
...
- 同品类该类型图片的常见做法
- 跨行业参考案例
- **视觉表达方案**：如何围绕核心传达组织信息元素（视觉层次、信息布局）
- 设计建议
### AI绘图Prompt
```
[详细prompt]
...
⚠️ **每张图的图片类型、核心传达、信息元素必须与规划严格一致。**

# 亚马逊Listing图片设计Brief

## 1. 项目概述
含图片整体策略说明：按消费者浏览心理旅程编排，每张图围绕一个核心传达组织信息

## 2. 消费者洞察总结

## 3. 图片设计需求（每张图）
- **图片类型**：[从规划中复述]
- **核心传达**：[本图要让消费者产生的感受/认知]
- **信息元素**：[画面中展示的具体细节/特征]
- **图片文案**（英文主标题3-6词体现核心传达 + 副标题/标注体现信息元素）
- **构图设计**（布局、角度、产品状态、背景——信息元素如何在画面中围绕核心传达组织）
- **信息层次**（核心传达对应主视觉区，信息元素如何分布）
- **文字排版**
- **色彩方案**
- **AI绘图Prompt**
...
**字节数**："""
    ...
    def _get_branch_extraction_config(self, branch, stage_num):
        ...
        c = {
            ("A", 0): ("IMAGE_PLAN", """提取图片规划。注意每张图有一个核心传达和若干信息元素。
Output in ```json format:
```json
{"images": [{"position": "主图/附图N", "image_type": "图片类型", "core_message": "核心传达（一句话）", "info_elements": ["信息元素1", "信息元素2"], "psychology_goal": "消费者心理目标"}], "ordering_logic": "排序逻辑", "coverage": [{"selling_point": "卖点名", "appears_in": ["主图", "附图2"], "has_core_image": true}]}
```""", ["images"]),

            ("A", 1): ("IMAGE_DESIGN", """提取表达方式和AI绘图prompt。注意每张图有核心传达和信息元素。
Output in ```json format:
```json
{"designs": [{"position": "主图/附图N", "image_type": "图片类型", "core_message": "核心传达", "info_elements": ["元素1", "元素2"], "visual_approach": "视觉表达方式", "ai_prompt": "prompt"}]}
```""", ["designs"]),
